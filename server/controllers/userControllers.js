...
userControllers.getLoginUrl = (req, res, next) => {
  ...
  const googleConfig = {
    clientId:
      "371087135-djckvfenrkntg92agsc5c7csq2d3cej1.apps.googleusercontent.com",
...
userControllers.getUser = (req, res, next) => {
  db.query(
    `SELECT username FROM users WHERE username = ${oauthObj.id}`, // MUST CHANGE ID PASSED INTO QUERY BASED ON GOOGLE OAUTH
    (err, results) => {
      if (err) return next(err);
      if (results === undefined) {
        // if not in db, go to post user
...
userControllers.postUser = (req, res, next) => {
  db.query(
    `INSERT INTO users (username, password, first_name, last_name) VALUES ('${oauthObj.username}', '${oauthObj.firstName}', '${oauthObj.lastName}')`,
    (err, results) => {
      if (err) return next(err);
      res.locals.userInfo = results;
...
userControllers.updateRoutine = (req, res, next) => {
  db.query(
    `INSERT INTO routine (users_id, repeat_every, repeat_frequency) VALUES (${req.body.usersId}, ${req.body.repeatEvery}, '${req.body.repeatFrequency}')`,
    (err, results) => {
      if (err) return next(err);
...
userControllers.updateUserHabits = (req, res, next) => {
  ...
  db.query(
    `INSERT INTO user_habits (users_id, habits_id, memo, routine_id, start_date, end_date, created_date) 
